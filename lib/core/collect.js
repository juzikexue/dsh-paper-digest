...
import { scoreTopicMatch } from './topic-match.js';
import { attachFullTexts } from './downloader.js';
import { summarizePapers } from './summarizer.js';
import { topicTerms, splitConcepts } from './config.js';

/**
 * Attach resolved keywords and concepts to a topic once per run.
 *
 * `terms` is the flat probe list the source adapters need — one request per
 * keyword. `concepts` carries the AND structure the relevance gate needs:
 * 分号 separates concepts (all required), 逗号 lists synonyms within one concept
 * (any one suffices).
 */
function withTerms(topic) {
  return {
    ...
    concepts: [...splitConcepts(topic.zh), ...splitConcepts(topic.en)],
  };
}
 ...
 * @param {object} cfg
 * @param {Date} [now]
 * @param {{onReject?: (paper: object, topic: object, score: number) => void}} [hooks]
 *   `onReject` receives every paper the relevance gate drops, together with the
 *   score it fell short by. Keeping them costs nothing and is the raw material
 *   for keyword-gap analysis (`lib/core/suggest.js`); without it a near-miss is
 *   indistinguishable from noise the moment it is discarded.
 * @returns {Promise<{papers: object[], stats: object[], errors: object[]}>}
 */
export async function collectCandidates(cfg, now = new Date(), hooks = {}) {
  const topics = cfg.topics.map(withTerms);
  const ctx = ctxFor(cfg, now);
  const stats = [];
  const errors = [];
  const all = [];
  const reject = hooks.onReject;
  ...
  for (const result of settled) {
    ...
    for (const paper of result.papers) {
      ...
      const match = matchesTopic(paper, result.topic);
      if (!match) {
        // Report the score it fell short by, so the rejection is measurable
        // rather than a silent boolean.
        if (reject) {
          const scored = scoreTopicMatch(paper, result.topic);
          reject(paper, result.topic, scored.score);
        }
        continue;
      }
      paper.matchedTerms = match;
      paper.topicId = result.topic.id;
      paper.topicName = result.topic.name;
      all.push(paper);
      kept += 1;
    }
...
export async function buildDigest(cfg, now = new Date(), options = {}) {
  const { papers, stats, errors } = await collectCandidates(cfg, now, options.hooks ?? {});
