...
function sanitiseTopics(raw, fallback) {
  ...
  for (const item of raw) {
    ...
    out.push({
      ...
      name: name || zh.split(/[;；]/)[0] || `主题${out.length + 1}`,
      zh,
      en,
    });
  }
  return out.length > 0 ? out.slice(0, 20) : fallback;
}
...
/**
 * A topic's keyword list is a free-text field: entries separated by `;`/`；`/
 * newline, and each entry may list alternatives with `,`/`，`/`、`. Every token
 * is returned separately so sources can use them as independent probes.
 */
export function splitKeywords(value) {
  return String(value ?? '')
    ...
    .map((s) => s.trim())
    .filter(Boolean)
    .flatMap((group) => group.split(/[,，、]+/).map((s) => s.trim()).filter(Boolean));
}
