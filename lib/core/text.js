 ...
 *
 * Two-tier judgement, because a single incidental word in a long abstract is
 * not evidence of a topic (verified: a Chinese paediatric paper about spinal
 * muscular atrophy matched "学习分析" purely because its abstract used the word
 * 学习, and was then selected for a learning-analytics digest):
 *
 *   1. the **title** satisfies any keyword group → accept;
 *   2. otherwise the body must satisfy at least `abstractGroups` distinct
 *      keywords, which a stray shared word cannot do.
 */
export function matchesTopic(paper, topic, options = {}) {
  ...
  return bodyHits.length >= minGroups ? bodyHits : null;
}
