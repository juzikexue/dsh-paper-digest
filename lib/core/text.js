...
import { matchesTopicScored, scoreTopicMatch } from './topic-match.js';
 ...
 *
 * Separator semantics follow the convention every bibliographic database uses:
 * **concepts are ANDed, synonyms within a concept are ORed**. A librarian writes
 * `(AI OR "artificial intelligence") AND (education OR teaching)`; the same
 * request here is `人工智能, AI; 教育, 教学`. Reading the separators the other way
 * round admits any paper that mentions 人工智能 even when nobody studied 教育 —
 * a false positive by that standard, and the shape of the mismatch measured on
 * live data (a 《图书情报工作》 AI paper entering an AI-in-education digest).
 *
 * Two tiers:
 *
 *   1. **strict** — a concept is satisfied when one of its synonyms matches the
 *      title, or when two distinct synonyms match the body; every concept must be
 *      satisfied. This is exact but brittle on wording alone (a single inserted
 *      word defeats a synonym: "Artificial Intelligence **in** Education").
 *   2. **scored** — `scoreTopicMatch` decomposes each synonym into tokens and
 *      accumulates field-weighted hits, so wording variants still count.
 *
 * Both tiers are ANDed across concepts. When a topic carries no concepts at all
 * (legacy `terms`-only callers, or a topic with no keywords) the older flat
 * behaviour is used, so nothing that worked before stops working.
 *
 * @param {object} paper
 * @param {object} topic with `concepts` (preferred) and/or `terms`
 * @param {object} [options] `mode` = 'strict' | 'scored' | 'mixed' (default)
 * @returns {string[]|string[][]|true|null} the concepts that matched, `true` for
 *   a topic with no keywords at all, or null
 */
export function matchesTopic(paper, topic, options = {}) {
  const concepts = conceptsOf(topic, options);
  if (concepts === null) {
    return legacyMatchesTopic(paper, topic, options);
  }
  if (concepts.length === 0) return true;
  const mode = options.mode ?? 'mixed';

  const strictResults = concepts.map((concept) => strictConcept(paper, concept, options));
  const scoredResults = concepts.map((concept) => {
    // No coverage floor by default: any overlap contributes and partial coverage
    // is charged by the scorer, which is what makes the tier tolerant of wording.
    // A caller may raise `conceptRatio` for a stricter per-concept bar.
    const result = scoreTopicMatch(paper, { ...topic, terms: { all: concept } }, {
      ...options,
      ratio: options.conceptRatio ?? 0,
    });
  ...
  for (let i = 0; i < concepts.length; i += 1) {
    const hit = mode === 'strict' ? strictResults[i] : mode === 'scored' ? scoredResults[i] : strictResults[i] ?? scoredResults[i];
    if (!hit) return null;
    out.push(hit);
  }
  return out;
...
/**
 * Concepts for a topic, or null when the caller supplied none and the legacy flat
 * path must be used instead.
 * @returns {string[][]|null}
 */
function conceptsOf(topic, options) {
  if (Array.isArray(options.concepts)) return options.concepts;
  if (Array.isArray(topic?.concepts)) return topic.concepts;
  if (Array.isArray(topic?.terms?.all) && topic.terms.all.length > 0) {
    // Legacy shape: each term is its own concept, which is the previous OR
    // behaviour only when there is a single term. Callers that still pass `terms`
    // get the old semantics verbatim.
    return null;
  }
  return [];
}

/** Tier 1 for one concept: title hit on a synonym, else two body hits. */
function strictConcept(paper, concept, options) {
  const ratio = options.ratio ?? 1;
  const titleBlob = normaliseTitle(`${paper.title} ${(paper.keywords ?? []).join(' ')}`);
  const titleHits = concept.filter((term) => termGroupHit(term, titleBlob, ratio) === 1);
  if (titleHits.length > 0) return titleHits;
  const body = normaliseTitle(`${paper.title} ${paper.abstract} ${(paper.keywords ?? []).join(' ')}`);
  const bodyHits = concept.filter((term) => termGroupHit(term, body, ratio) === 1);
  const need = options.abstractGroups ?? 2;
  return bodyHits.length >= need ? bodyHits : null;
}

/** The pre-concept behaviour, kept for callers that pass a flat `terms` list. */
function legacyMatchesTopic(paper, topic, options = {}) {
