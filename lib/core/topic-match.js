...
/** How many of a term's tokens appear anywhere in the record. */
function coverageOf(tokens, blobs) {
  const hit = tokens.filter((token) => blobs.some((blob) => blob.includes(token)));
  return { found: hit.length, total: tokens.length, ratio: tokens.length === 0 ? 0 : hit.length / tokens.length };
}

/**
 * Continuous relevance of one paper to one topic.
 *
 * Scoring is **field-weighted token coverage**, not all-or-nothing: every token
 * of a keyword that appears in the title is worth `FIELD_WEIGHT.title`, one that
 * appears in the abstract worth less, and the field is credited at its best
 * coverage. A Chinese synonym contributes several overlapping windows, so
 * 《人工智能辅助语文写作教学研究》 earns most of a 语文教育 concept without
 * containing that exact four characters.
 *
 * The earlier form required *every* token of a term and subtracted a penalty
 * otherwise, which meant a partial match could never reach the threshold — under
 * concept-AND that made the gate unusable, since both concepts then had to be
 * matched verbatim.
 *
 * `options.ratio` (default 1) is the coverage a token needs to count, so a caller
 * can loosen the per-concept bar while the AND across concepts stays strict.
 *
 * @param {object} paper normalised paper record
 * @param {object} topic topic carrying `terms.all` or an explicit `options.terms`
...
export function scoreTopicMatch(paper, topic, options = {}) {
  const terms = options.terms?.all ?? topic?.terms?.all ?? [];
  const minScore = options.minScore ?? DEFAULT_MIN_SCORE;
  // Fraction of a term's tokens that must be present before the term counts.
  // Defaults to 0: *any* overlap contributes, which is what makes the scored tier
  // tolerant of wording. A concept-AND caller raises it (see `conceptRatio` in
  // text.js) so that "one shared window" cannot satisfy a whole required concept.
  const tokenRatio = options.ratio ?? 0;
  ...
  const fields = [
    ['title', normSpaced(paper?.title), FIELD_WEIGHT.title],
    ['keywords', normSpaced((paper?.keywords ?? []).join(' ')), FIELD_WEIGHT.keywords],
    ['abstract', normSpaced(paper?.abstract), FIELD_WEIGHT.abstract],
  ];
  const allBlobs = [...fields.map(([, blob]) => blob), normSpaced(paper?.venue)];
  ...
  for (const term of terms) {
    ...
    const venueBlob = normSpaced(paper?.venue);
    if (venueBlob) {
      const venueRatio = termFieldScore(term, venueBlob).ratio;
    ...
    for (const [field, blob, weight] of fields) {
      const { ratio } = termFieldScore(term, blob);
      if (ratio === 0) continue;
      // Full coverage keeps the field's weight; partial coverage keeps its share
      // and is charged the shortfall. Without the charge a single shared window
      // (`人工智能` inside a 人工智能 教育 keyword) scored the same as the whole
      // keyword and walked through the gate — the regression tests in
      // test/text.test.js exist precisely to stop that.
      const value = weight * ratio - PARTIAL_PENALTY * (1 - ratio);
      if (value > best) {
        best = value;
        bestField = field;
      }
    }
    // Also try the equivalent table, always — not only as a fallback. A partial
    // literal match can be weaker than a full equivalent one: "AI in Education"
    // shares two of three tokens with `artificial intelligence education` (title
    // coverage 0.67 → 2.5) but satisfies its equivalent form completely (→ 4.5).
    for (const equivalent of equivalentsOf(term)) {
      for (const [field, blob, weight] of fields) {
        const { ratio } = termFieldScore(equivalent, blob);
        if (ratio < 1) continue;
  ...
  // Coverage floor: a term counts only when at least `tokenRatio` of its tokens
  // were found. This is the knob a concept-AND caller loosens.
  const viable = matchedTerms.filter((term) => {
    const tokens = termTokens(term);
    return coverageOf(tokens, allBlobs).ratio >= tokenRatio;
  });

  return {
    score: Number(score.toFixed(2)),
    accepted: score >= minScore && viable.length > 0,
    matchedTerms: viable,
    reasons,
    fields: { title: Boolean(fields[0][1]), abstract: Boolean(fields[2][1]) },
  };
}
