...
/**
 * The single in-flight run, or null. `running` used to be a bare boolean, which
 * told the panel THAT a run was in flight but never since when or why — so the
 * boot catch-up (which holds this lock for minutes) was indistinguishable from
 * a hung one. Keep the identity here and derive the boolean from it.
 * @type {{startedAt: string, startedMs: number, trigger: string}|null}
 */
let runState = null;
...
function currentRunPayload() {
  ...
  return {
    ...
    elapsedMs: Date.now() - runState.startedMs,
  };
}

/**
 * Whether a run is in flight. Read through a function so this module does not
...
export async function runDigest(cfg, trigger = 'manual', services = {}) {
  if (runState) return { alreadyRunning: true, run: currentRunPayload(), config: cfg, markdown: '' };
  const started = Date.now();
  const now = new Date();
  const startedAt = now.toISOString();
  runState = { startedAt, startedMs: started, trigger };
  if (trigger !== 'manual') {
    console.log(`[dsh-paper-digest] ${trigger} run started · ${startedAt}`);
  }
  try {
...
function handleRoute(req, res, services) {
  ...
  if (action === 'run' && method === 'POST') {
    // An in-flight run is NOT an error: the panel's button and the scheduler's
    // boot catch-up compete for the same slot, and refusing the click with a
    // 409 read as "the plugin is broken" while the catch-up was in fact busy
    // producing exactly the report the click asked for. Answer with which run
    // is in flight and let the caller track it.
    if (runState) {
      ...
      sendJson(res, 202, {
        ...
        message:
          run.trigger === 'schedule'
            ? '今日的定时检索已在进行中，正在跟踪这一次运行'
            : '已有一次运行在进行中，正在跟踪这一次运行',
      });
      return;
    }
    const cfg = readConfig();
...
function startScheduler(ctx, services) {
  const tick = () => {
    try {
      ...
      runDigest(cfg, 'schedule', services).then((result) => {
        // A manual run that got there first already marks the day; dropping the
        // catch-up is correct, and saying so beats a silent no-op.
        if (result?.alreadyRunning) {
          console.log('[dsh-paper-digest] scheduled run skipped: another run is already in flight');
        }
      }).catch((error) => {
