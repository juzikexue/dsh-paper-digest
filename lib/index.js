    ...
    readJsonBody(req)
      .then(async (body) => {
        ...
        if (body?.topicId !== undefined || body?.keywordsFor !== undefined) {
          ...
          // Check what each proposal actually retrieves, so the user is never
          // asked to trust a phrase that merely sounds plausible.
          const coverage = await checkKeywordCoverage(groups.map((g) => g.en), {
          ...
          sendJson(res, 200, {
            ...
            topicId: topic.id ?? '',
            groups: groups.map((g) => {
              const count = coverage.get(g.en) ?? null;
              return { ...g, coverage: count, verdict: coverageVerdict(count) };
            }),
            route: answer.route,
