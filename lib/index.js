...
import { completeOnce } from './core/summarizer.js';
import {
  mineGap,
  buildSuggestPrompt,
  parseSuggestions,
...
/* ------------------------------------------------- rejected-candidate pool -- */

/**
 * Papers the relevance gate dropped during the most recent run, newest run only.
 *
 * Kept in memory rather than on disk: it is regenerated by every run, it is only
 * read when the user asks for keyword suggestions, and writing a few hundred
 * abstracts per day into the plugin's storage would grow without bound.
 */
let lastRejected = [];

/** Suggestion state: pending proposals plus the phrases the user threw away. */
function suggestFile() {
  return join(storeDir(), 'suggestions.json');
}

function readSuggestState() {
  try {
    const parsed = JSON.parse(readFileSync(suggestFile(), 'utf8'));
    return {
    ...
    return { generatedAt: '', topicId: '', suggestions: [], dismissed: [] };
  }
}

function writeSuggestState(state) {
  try {
    mkdirSync(storeDir(), { recursive: true });
    const tmp = `${suggestFile()}.${process.pid}.tmp`;
    writeFileSync(tmp, `${JSON.stringify(state, null, 2)}\n`, 'utf8');
    renameSync(tmp, suggestFile());
  } catch {
    /* suggestions are advisory; never fail a request over persistence */
  }
  return state;
}
...
/**
 * Mine the last run's rejected pool for keyword suggestions.
 *
 * Grounded by construction: the candidate phrases come from papers the user's
 * own retrieval already fetched, and the model is asked to quote them rather
 * than invent them. Nothing is applied automatically — the panel shows each
 * proposal as a chip, and the user adopts or discards it.
 *
 * @param {object} cfg
 * @param {{llm?: object, agentDefaultModel?: object}} services
 * @returns {Promise<object>} the analyse result, also persisted to the store
 */
async function runSuggestionAnalysis(cfg, services) {
  const state = readSuggestState();
  if (lastRejected.length === 0) {
    return {
      ok: false,
      error: '还没有可分析的候选：先运行一次日报，本次被过滤掉的论文才会进入分析池',
      state,
      poolSize: 0,
    };
  }

  const { nearMisses, controls, topicId } = mineGap(lastRejected);
  const topic = cfg.topics.find((t) => t.id === topicId);
  if (!topic || nearMisses.length === 0) {
    return { ok: false, error: '没有可用于分析的候选', state, poolSize: lastRejected.length };
  }

  const prompt = buildSuggestPrompt({ topic, nearMisses, controls });
  const answer = await completeOnce({
    llm: services.llm,
    agentDefaultModel: services.agentDefaultModel,
    prompt,
    system:
      '你是学术检索的查询扩展助手。你只依据用户提供的论文文本作答，绝不使用自己的知识补充未出现的概念。只输出 JSON。',
    maxTokens: cfg.summaryMaxTokens || 4000,
    timeoutMs: Math.max(cfg.requestTimeoutMs, 60000),
    reasoningEffort: cfg.summaryReasoningEffort || '',
  });
  if (!answer.ok) {
    return { ok: false, error: answer.error, state, poolSize: lastRejected.length };
  }

  const corpus = [...nearMisses, ...controls].map((r) => r.paper);
  const suggestions = parseSuggestions(answer.text, { topic, corpus });
  const next = writeSuggestState(
    mergeSuggestions({ ...state, topicId }, suggestions, topicId),
  );
  console.log(
    `[dsh-paper-digest] keyword gap: pool=${lastRejected.length} near=${nearMisses.length} suggestions=${suggestions.length}`,
  );
  return {
    ok: true,
    state: next,
    poolSize: lastRejected.length,
    nearMissCount: nearMisses.length,
    topic: { id: topic.id, name: topic.name },
    route: answer.route,
    raw: suggestions.length === 0 ? String(answer.text).slice(0, 600) : '',
  };
}
...
export async function runDigest(cfg, trigger = 'manual', services = {}) {
  ...
  try {
    // Every paper the relevance gate drops is kept for keyword-gap analysis.
    // The pool is reset per run so a stale corpus never backs a suggestion.
    lastRejected = [];
...
function handleRoute(req, res, services) {
  ...
  if (action === 'suggest' && method === 'POST') {
    readJsonBody(req)
      .then(async (body) => {
        if (body?.dismiss) {
          // A discarded phrase must not come back on the next analysis, or the
          // panel turns into a nag.
          const state = readSuggestState();
          const key = String(body.dismiss).trim().toLowerCase().replace(/\s+/g, ' ');
          const dismissed = [...new Set([...(state.dismissed ?? []), key])];
          const next = writeSuggestState({
            ...state,
            dismissed,
            suggestions: (state.suggestions ?? []).filter(
              (s) => String(s.phrase).trim().toLowerCase().replace(/\s+/g, ' ') !== key,
            ),
          });
          sendJson(res, 200, { ok: true, state: next });
          return;
        }
        // Keywords from a topic name: independent of any run, so this works on a
        // brand-new topic before the first digest has ever executed.
        if (body?.topicId !== undefined || body?.keywordsFor !== undefined) {
          const cfg = readConfig();
          const byId = body.topicId ? cfg.topics.find((t) => t.id === body.topicId) : null;
          const typedName = String(body.keywordsFor ?? '').trim();
          const topic = byId ?? (typedName ? { name: typedName } : null);
          if (!topic || !topic.name) {
            sendJson(res, 400, { ok: false, error: '请先填写主题名，再生成关键词' });
            return;
          }
          const answer = await completeOnce({
            llm: services.llm,
            agentDefaultModel: services.agentDefaultModel,
            prompt: buildKeywordPrompt({ name: topic.name }),
            system:
              '你是学术检索的关键词设计助手。你只输出符合要求的 JSON，不解释、不寒暄、不写检索式语法。',
            maxTokens: Math.min(cfg.summaryMaxTokens || 4000, 4000),
            timeoutMs: Math.max(cfg.requestTimeoutMs, 60000),
            reasoningEffort: cfg.summaryReasoningEffort || '',
          });
          if (!answer.ok) {
            sendJson(res, 502, { ok: false, error: answer.error, topicId: topic.id ?? '' });
            return;
          }
          const groups = parseKeywordGroups(answer.text);
          // Check what each English phrasing actually retrieves, so the user is
          // never asked to trust a phrase that merely sounds plausible.
          const enPhrasings = groups
            .flatMap((g) => String(g.en ?? '').split('+'))
            .map((s) => s.trim())
            .filter(Boolean);
          const coverage = await checkKeywordCoverage(enPhrasings, {
          ...
          sendJson(res, 200, {
            ...
            topicId: topic.id ?? '',
            groups: groups.map((g) => {
              // The requirement ANDs its phrasings, so the weakest phrasing is
              // what bounds how much the set can retrieve.
              const counts = enPhrasings.map((phrase) => coverage.get(phrase) ?? null);
              const known = counts.filter((c) => c !== null);
              const worst = counts.some((c) => c === null)
                ? null
                : known.length
                  ? Math.min(...known)
                  : null;
              return {
                ...g,
                coverage: worst,
                verdict: coverageVerdict(worst),
                perPhrasing: enPhrasings.map((phrase) => ({
                  phrase,
                  count: coverage.get(phrase) ?? null,
                  verdict: coverageVerdict(coverage.get(phrase) ?? null),
                })),
              };
            }),
            route: answer.route,
            raw: groups.length === 0 ? String(answer.text).slice(0, 600) : '',
          });
          return;
        }
        const cfg = readConfig();
        const result = await runSuggestionAnalysis(cfg, services);
...
export function apply(ctx) {
  ...
  for (const sub of ['/config', '/status', '/run', '/defaults', '/core-list', '/open-session', '/suggest']) {
    ctx.effect(
      () =>
        ctx.webServer.register({
          kind: 'exact',
